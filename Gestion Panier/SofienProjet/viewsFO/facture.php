...
While ($num_page <= $nb_page)
{
    ...
    if ($num_page == $nb_page)
    {
        ...
        $x = 130;
        $sql = 'select Prix_Produit,sum( round(Prix_Produit * Qty_Produit,2) ) tot_ht FROM facture where ID_Commande=' .$var_id_facture;
        $res = mysqli_query($mysqli, $sql)  or die ('Erreur SQL : ' .$sql .mysqli_connect_error() );
        while ($data =  mysqli_fetch_assoc($res))
        {
            $pdf->SetXY( $x, 221 ); $pdf->Cell( 17, 6, $data['Prix_Produit'] . ' %', 0, 0, 'C');
        ...
        $nombre_format_francais = "Net à payer TTC : " . number_format($data['tot_ht'], 2, ',', ' ') . " €";
